...
export default function InvitationsPage() {
  ...
  const occasions = [
    { title: "زواج", href: "wedding", count: 20 },
    { title: "تخرج", href: "graduation", count: 14 },
    { title: "مواليد", href: "newborn", count: 11 },
    { title: "عيد ميلاد", href: "birthday", count: 18 },
    { title: "خطوبة", href: "engagement", count: 16 },
  ]

  const getOccasionHref = (occasionHref: string) => {
    if (selectedFilter === null || selectedFilter === "all") {
      return `/services/invitations/${occasionHref}`
    }
    const is3D = selectedFilter === "3d"
    return `/services/invitations/${occasionHref}?filter=design&value=${is3D}`
  }
      ...
      <main className="pt-20 px-4 max-w-screen-xl mx-auto">
        ...
        {selectedFilter === null ? (
          <div className="space-y-4">
            <p className="text-[#B38C8A] text-center mb-8">اختر نوع الدعوة المناسبة لك</p>
              ...
              {filters.map((filter) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-gradient-to-br from-[#8B7355] to-[#6D5A43] rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
          ...
          <div>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-semibold text-[#B38C8A]">
                الدعوات - {filters.find((f) => f.id === selectedFilter)?.label}
              </h2>
              <button
