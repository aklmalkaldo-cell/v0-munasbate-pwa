...
export default function ZaffatPage() {
  ...
  const filters = [
    ...
    { id: "no_music", label: "بدون موسيقى", icon: VolumeX, value: false },
    { id: "all", label: "عرض الكل", icon: List, value: null },
  ]

  const occasions = [
    { title: "زواج", href: "wedding", count: 12 },
    { title: "تخرج", href: "graduation", count: 8 },
    { title: "مواليد", href: "newborn", count: 6 },
    { title: "نجاح", href: "success", count: 5 },
    { title: "ترقية", href: "promotion", count: 4 },
  ]

  const getOccasionHref = (occasionHref: string) => {
    if (selectedFilter === null || selectedFilter === "all") {
      return `/services/zaffat/${occasionHref}`
    }
    const hasMusic = selectedFilter === "with_music"
    return `/services/zaffat/${occasionHref}?filter=music&value=${hasMusic}`
  }
      ...
      <main className="pt-20 px-4 max-w-screen-xl mx-auto">
        ...
        {selectedFilter === null ? (
          ...
          <div className="space-y-4">
            <p className="text-[#B38C8A] text-center mb-8">اختر نوع الزفة المناسبة لك</p>
              ...
              {filters.map((filter) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-gradient-to-br from-[#D4AF37] to-[#C49F27] rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
          ...
          <div>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-semibold text-[#B38C8A]">
                الزفات - {filters.find((f) => f.id === selectedFilter)?.label}
              </h2>
              <button
